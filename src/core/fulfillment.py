...
# Параметры подписок.
GB = 1024 ** 3
REGULAR_TRAFFIC_GB = 100    # обычная подписка
...
def _sync_user_status(user_id: int) -> None:
    user = db.fetchone("SELECT is_banned FROM users WHERE id = ?", (user_id,))
    if not user or user.get("is_banned"):
        return
    active = db.fetchone(
        "SELECT type FROM subscriptions WHERE user_id = ? AND status = 'Active' "
        "ORDER BY CASE WHEN expires_at IS NULL THEN 1 ELSE 0 END, expires_at DESC LIMIT 1",
        (user_id,),
    )
    status = "Trial" if (not active or active.get("type") == "trial") else "Active"
...
def grant_trial(user: dict[str, Any]) -> dict[str, Any]:
    """
    Выдаёт бесплатную пробную подписку: 3 дня, 5 ГБ, HWID 1, сброс «Ежемесячно».
    Вызывающий обязан проверить, что триал ещё не использовался и что он включён.
    """
    telegram_id = int(user["telegram_id"])
    new_exp = utcnow() + timedelta(days=TRIAL_DAYS)
    prov = provisioning.provision(
        telegram_id=telegram_id, username=user.get("username"), expire_at=new_exp,
        devices=TRIAL_DEVICES, squads=trial_squads(), email=user.get("email"),
        traffic_limit_bytes=TRIAL_TRAFFIC_GB * GB, traffic_reset_strategy=RESET_STRATEGY,
    )
    sub = _create_subscription_row(
        user=user, devices=TRIAL_DEVICES, expire_at=new_exp, provision=prov,
        traffic_gb=TRIAL_TRAFFIC_GB, sub_type="trial", squads=trial_squads(),
    )
...
def grant_subscription(
    *,
    ...
    if purpose == "devices" and subscription_id:
        ...
        prov = provisioning.provision(
            telegram_id=telegram_id, username=username, expire_at=expire_dt,
    ...
    if target:
        ...
        prov = provisioning.provision(
            telegram_id=telegram_id, username=username, expire_at=new_exp,
            devices=new_devices, squads=vpn_squads(), email=email,
            traffic_limit_bytes=REGULAR_TRAFFIC_GB * GB, traffic_reset_strategy=RESET_STRATEGY,
        ...
        db.execute(
            ...
            (iso(new_exp), new_devices, REGULAR_TRAFFIC_GB, 1 if upgrading_trial else 0,
    ...
    sub = _create_subscription_row(
        ...
        traffic_gb=REGULAR_TRAFFIC_GB, sub_type="vpn",
    )
    return {
        "subscription_id": sub["id"],
        "expires_at": sub.get("expires_at"),
